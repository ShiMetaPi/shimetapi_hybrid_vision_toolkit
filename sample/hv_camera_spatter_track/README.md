# HV Camera Spatter Track Demo

## 功能描述

这个示例程序结合了HV相机的实时显示和飞溅颗粒检测功能，提供了一个交互式的界面来控制数据采集和事件处理。
...
make
```
或者可以使用hv_camera_spatter_track目录下的`build.sh`脚本进行编译
...
```bash
# 使用默认USB设备ID
./hv_camera_spatter_track

# 或指定USB设备ID（十六进制）
./hv_camera_spatter_track 1d6b 0105
```
